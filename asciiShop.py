...
class AsciiImage(RandomCat):
    ...
    """
    converts random cat photo to an array of pixel values then takes 
    that array and changes each pixel ti an ascii char
    """
...
@Name: flip
@Description: flips image either verticallyor horizontally
...
"""      
    def flip(self, direction):
        if (direction == "vertical"):
            ...
            #all pixels in row 0 -> row N, row 1 ->row n-1, row n/2 -> row n/2+1 
            for c in range(0, len(self.imageAsAscii), self.newWidth):
                self.imageAsAscii[c] = self.newWidth - c
            return self.imageAsAscii
        else:
            #flip picture horizontally
            #all pixels in col 0 -> col n, col 1 -> coln-1, col n/2 -> coln/2+1
            return self.imageAsAscii
...
"""
@Name: invert
@Description: switches lightest and darkest pixels to invert image
@Params: none
@returns: inverted ascii image 
 """           
    def invert(self):
        self.asciiChars.reverse() #reverse the indeces of the asciiChars
        #print (self.asciiChars)
       # printImage()
